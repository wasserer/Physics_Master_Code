...
FIGSIZE = (5, 3)
NORMALIZED_FIGSIZE = (8, 4)
NORMALIZED_SCATTER_SIZE = 5
PLOT_DPI = 300
...
TC_LOG_ROOT = px1_file.resolve().parents[1]
EXPORT_ROOT = TC_LOG_ROOT / f'RESULT_NORMALIZED_{TIMESTAMP}'
metric_filenames = {
...
NORMALIZED_PLOT_ROOT = EXPORT_ROOT / 'normalized_plots'
NORMALIZED_PLOT_ROOT.mkdir(parents=True, exist_ok=True)
...
NORMALIZED_RESISTANCE_METRICS = {
    'Rs [Ω·cm²]': 'Rs_avg',
    'Rp [Ω·cm²]': 'Rp_avg',
}
NORMALIZED_RESISTANCE_COLORS = {
    'Rs [Ω·cm²]': '#17becf',
    'Rp [Ω·cm²]': '#bcbd22',
}
NORMALIZED_RESISTANCE_LABELS = {
    'Rs [Ω·cm²]': 'R$_s$',
    'Rp [Ω·cm²]': 'R$_p$',
}

TARGET_TEMPERATURE_LABEL = '70_80'
try:
    TARGET_TEMPERATURE_RANGE = dict(TEMPERATURE_RANGES)[TARGET_TEMPERATURE_LABEL]
except KeyError as exc:
    raise ValueError(
        f'Target temperature range label "{TARGET_TEMPERATURE_LABEL}" is not defined.'
    ) from exc
TARGET_TEMPERATURE_RANGE_STR = format_temp_range(*TARGET_TEMPERATURE_RANGE)
...
for pixel_name, avg_df in px_cycle_avg_by_temp.items():
    ...
    for metric, alias in cycle_metrics.items():
        ...
        fig.savefig(figure_path, dpi=PLOT_DPI)
        plt.close(fig)
...
for pixel_name, avg_df in px_cycle_avg_by_temp.items():
    ...
    normalized_value_series = []
    for metric, alias in NORMALIZED_METRICS.items():
        mean_col = f'{alias}_{TARGET_TEMPERATURE_LABEL}'
        err_col = f'{alias}_err_{TARGET_TEMPERATURE_LABEL}'
        if mean_col not in avg_df.columns:
            continue
        series = avg_df[mean_col]
        raw_errors = err_df.get(
            err_col,
            pd.Series(np.nan, index=avg_df.index),
        )
        raw_errors = raw_errors.reindex(series.index)
        baseline_mask = (cycle_numbers == 1) & series.notna()
        if not baseline_mask.any():
            continue
        baseline_value = series[baseline_mask].iloc[0]
        if pd.isna(baseline_value) or baseline_value == 0:
            continue
        normalized_series = series / baseline_value
        normalized_errors = raw_errors / baseline_value
        normalized_metric_data[metric] = (normalized_series, normalized_errors)
        normalized_value_series.append(normalized_series)

    if not normalized_metric_data:
        continue

    fig, ax = plt.subplots(figsize=NORMALIZED_FIGSIZE)
    for metric, (series, errors) in normalized_metric_data.items():
        color = NORMALIZED_METRIC_COLORS.get(metric, '#000000')
        label = NORMALIZED_METRIC_LABELS.get(metric, metric)
        label = label.replace('[', '').replace(']', '')
        mask = series.notna()
        x_values = cycle_numbers[mask]
        y_values = series[mask]
        y_errors = errors.reindex(series.index)[mask].fillna(0)
        ax.plot(
            cycle_numbers,
            series,
            linestyle='-',
    ...
    combined_values = pd.concat(normalized_value_series) if normalized_value_series else pd.Series(dtype=float)
    combined_values = combined_values.dropna()
    if not combined_values.empty:
        upper = max(1.05, combined_values.max() * 1.05)
        lower = min(0, combined_values.min() * 0.95)
        ax.set_ylim(lower, upper)
    ax.grid(True, which='both', linestyle='--', linewidth=0.5)
    ax.legend()
    plt.tight_layout()

    figure_path = NORMALIZED_PLOT_ROOT / f'{pixel_name}_{TARGET_TEMPERATURE_LABEL}_normalized_metrics.png'
    fig.savefig(figure_path, dpi=PLOT_DPI)
    plt.close(fig)
...
for pixel_name, avg_df in px_cycle_avg_by_temp.items():
    cycle_numbers = avg_df['Cycle Number']
    err_df = px_cycle_err_by_temp[pixel_name]
    normalized_resistance_data = {}
    normalized_resistance_series = []
    for metric, alias in NORMALIZED_RESISTANCE_METRICS.items():
        mean_col = f'{alias}_{TARGET_TEMPERATURE_LABEL}'
        err_col = f'{alias}_err_{TARGET_TEMPERATURE_LABEL}'
        if mean_col not in avg_df.columns:
            continue
        ...
        normalized_series = series / baseline_value
        normalized_errors = raw_errors / baseline_value
        normalized_resistance_data[metric] = (normalized_series, normalized_errors)
        normalized_resistance_series.append(normalized_series)

    if not normalized_resistance_data:
        continue

    fig, ax = plt.subplots(figsize=NORMALIZED_FIGSIZE)
    for metric, (series, errors) in normalized_resistance_data.items():
        color = NORMALIZED_RESISTANCE_COLORS.get(metric, '#000000')
        label = NORMALIZED_RESISTANCE_LABELS.get(metric, metric)
        mask = series.notna()
        x_values = cycle_numbers[mask]
        y_values = series[mask]
        y_errors = errors.reindex(series.index)[mask].fillna(0)
    ...
    ax.set_xlabel('Cycle Number')
    ax.set_ylabel('Normalized Resistance Value')
    combined_values = (
        pd.concat(normalized_resistance_series)
        if normalized_resistance_series
    ...
    figure_path = NORMALIZED_PLOT_ROOT / f'{pixel_name}_{TARGET_TEMPERATURE_LABEL}_normalized_resistance.png'
    fig.savefig(figure_path, dpi=PLOT_DPI)
    plt.close(fig)
