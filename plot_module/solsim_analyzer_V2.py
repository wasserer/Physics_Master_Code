...
class solarSimulator:
    ...
    def calcIsc(self, voltage, current):
        try:
            ...
            num_points = min(10, voltage.size)
            idx = np.argsort(np.abs(voltage))[:num_points]
            v_sel = voltage[idx]
            c_sel = current[idx]

            if v_sel.size < 2 or np.allclose(v_sel, v_sel[0]):
                fallback_idx = np.argmin(np.abs(voltage))
                return current[fallback_idx]

            slope, intercept = np.polyfit(v_sel, c_sel, 1)
            if np.isclose(slope, 0.0):
                fallback_idx = np.argmin(np.abs(voltage))
                return current[fallback_idx]

            Isc = intercept  # linear-fit intersection with V=0
            return Isc
    ...
    def calcVoc(self, voltage, current):
        try:
            ...
            num_points = min(10, voltage.size)
            idx = np.argsort(np.abs(current))[:num_points]
            v_sel = voltage[idx]
            c_sel = current[idx]

            if v_sel.size < 2 or np.allclose(v_sel, v_sel[0]):
                fallback_idx = np.argmin(np.abs(current))
                return -voltage[fallback_idx]
            ...
            Voc = intercept / slope  # linear-fit intersection with I=0
            return Voc
        except Exception as e:
            print(f"Error:", e)
            return 0
    ...
    def calcRsRp(self, voltage, current):
        try:
        #Change the voltage and current into the np array
            voltage = np.asarray(voltage, dtype=float).ravel()
            current = np.asarray(current, dtype=float).ravel()
        #Find the valid mask(Filter out unwanted NaN):
            valid_mask = ~(np.isnan(voltage) | np.isnan(current))
            voltage = voltage[valid_mask]
            current = current[valid_mask]
            ...
            if voltage.size < 2 or current.size < 2:
                ...
                Rp = np.nan
                return Rs, Rp
        #Smooth current to suppress noise before extracting slopes
            smooth_window = min(30, current.size)
            if smooth_window > 1:
                kernel = np.ones(smooth_window) / smooth_window
                current_smooth = np.convolve(current, kernel, mode="same")
            else:
                current_smooth = current.copy()
        #Set up the window size
            rs_window_size = min(5, voltage.size)
        #Part where Rp was calculated:
            low_current_mask = (np.abs(voltage) > -0.6) & (np.abs(voltage) < 0.05)
            v_sc = voltage[low_current_mask]
            i_sc = current_smooth[low_current_mask]
            if v_sc.size < 2 or np.allclose(v_sc, v_sc[0]):
                slope_ic = np.nan
            else:
                slope_ic = np.polyfit(v_sc, i_sc, 1)[0] #Range with |I| in (0, 0.4) A
            Rp = np.inf if np.isnan(slope_ic) or np.isclose(slope_ic, 0.0) else np.abs(1.0 / slope_ic)
            if not np.isnan(Rp) and not np.isinf(Rp) and Rp > 20:
                Rp = np.nan
        #Part where the Rs was calculated
            near_oc_idx = np.argsort(np.abs(current_smooth))[:rs_window_size]
            i_oc = current_smooth[near_oc_idx]
            v_oc = voltage[near_oc_idx]
            if i_oc.size < 2 or np.allclose(i_oc, i_oc[0]):
                slope_vi = np.nan
            else:
                slope_vi = np.polyfit(i_oc, v_oc, 1)[0]
            Rs = np.inf if np.isnan(slope_vi) or np.isclose(slope_vi, 0.0) else np.abs(slope_vi)
    ...
    def calcRsRp_old(self, voltage, current):
        try:
        #Change the voltage and current into the np array
            voltage = np.asarray(voltage, dtype=float).ravel()
            current = np.asarray(current, dtype=float).ravel()
        #Find the valid mask(Filter out unwanted NaN):
            ...
            if voltage.size < 2 or current.size < 2:
                ...
                return Rs, Rp
        #Set up the window size
            rp_window_size = min(10, voltage.size)
            rs_window_size = min(10, voltage.size)
        #Part where Rp was calculated:
            near_sc_idx = np.argsort(np.abs(voltage))[:rp_window_size]
            v_sc = voltage[near_sc_idx]
            i_sc = current[near_sc_idx]
            if v_sc.size < 2 or np.allclose(v_sc, v_sc[0]):
                slope_ic = np.nan
            else:
                slope_ic = np.polyfit(v_sc, i_sc, 1)[0] #Range around the Isc
            Rp = np.inf if np.isnan(slope_ic) or np.isclose(slope_ic, 0.0) else np.abs(1.0 / slope_ic)
            if not np.isnan(Rp) and not np.isinf(Rp) and Rp > 20:
                Rp = np.nan
        #Part where the Rs was calculated
            near_oc_idx = np.argsort(np.abs(current))[:rs_window_size]
            i_oc = current[near_oc_idx]
            v_oc = voltage[near_oc_idx]
            if i_oc.size < 2 or np.allclose(i_oc, i_oc[0]):
                slope_vi = np.nan
            else:
    ...
    def loadFileData(self):
        ...
        try:
            self.Rs, self.Rp = self.calcRsRp(self.voltages, self.currents)
        except Exception:
            self.Rs, self.Rp = np.nan, np.nan
        self.labels = self.filePath

    def loadFileData_Cycling(self):
        ...
        files = pd.read_csv(
            ...
            skiprows=range(0, 19),
            header=None,
            names=['voltage', 'current', 'power', 'time'],
            sep=';'
        )
        self.data = np.asarray(files)
        self.data = self.data.T
