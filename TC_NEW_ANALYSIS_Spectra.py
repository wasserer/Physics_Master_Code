...
def collect_new_spectra_exports(
    spectra_folder: Path,
    result_dir: Path,
    created_after: float | None = None,
) -> None:
    """Copy freshly generated Spectra exports into the analysis result folder."""
    source_dir = spectra_folder / "Results"
    if not source_dir.exists():
        return

    dest_dir = result_dir / "spectra_exports"
    ...
    for csv_path in sorted(source_dir.glob("*.csv")):
        try:
            ...
            continue
        if created_after is not None and mtime < created_after - 1:
            continue
        target_path = dest_dir / csv_path.name
        shutil.copy2(csv_path, target_path)


def _gather_group_stats(values, sources, group_map):
    grouped = {}
    for source, value in zip(sources, values):
        key = group_map.get(source)
        if not key:
            continue
        try:
            val = float(value)
    ...
    for key, arr in grouped.items():
        ...
        if arr_np.size == 0:
            continue
        if arr_np.size > 1:
            std_val = float(np.std(arr_np, ddof=1))
        else:
            std_val = 0.0
        sem_val = float(std_val / np.sqrt(arr_np.size)) if arr_np.size else float("nan")
        mean_val = float(np.mean(arr_np))
        stats[key] = {"mean": mean_val, "std": std_val, "sem": sem_val}
    return stats


def _tauc_group_stats(spectra: Spectroscopy) -> dict[str, dict[str, float]]:
    raw = getattr(spectra, "tauc_results", {}).get("raw")
    if not raw:
        return {}
    group_map = getattr(spectra, "file_group_map", {})
    return _gather_group_stats(raw.get("bandGap", []), raw.get("sources", []), group_map)


def _urbach_group_stats(spectra: Spectroscopy) -> dict[str, dict[str, float]]:
    raw = getattr(spectra, "urbach_results", {}).get("raw")
    if not raw:
        return {}
    group_map = getattr(spectra, "file_group_map", {})
    return _gather_group_stats(raw.get("urbach_energy", []), raw.get("sources", []), group_map)
...
def log_urbach_metrics(
    ...
    group_map = getattr(spectra, "file_group_map", {})
    urbach_stats = _urbach_group_stats(spectra)
    sem_values = []
    for source in sources:
        key = group_map.get(source)
        stats = urbach_stats.get(key, {}) if key else {}
        sem_values.append(stats.get("sem", np.nan))
...
def log_tauc_metrics(
    ...
    group_map = getattr(spectra, "file_group_map", {})
    tauc_stats = _tauc_group_stats(spectra)
    sem_values = []
    for source in sources:
        key = group_map.get(source)
        stats = tauc_stats.get(key, {}) if key else {}
        sem_values.append(stats.get("sem", np.nan))
...
def main() -> None:
    ...
    collect_new_spectra_exports(spectra_folder, result_dir, created_after=run_start)
